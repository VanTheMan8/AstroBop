...
public class GenerateAsroBop : MonoBehaviour {

	[Header("Prefabs")]
	public Transform[] eyePrefabs;
	public Transform[] mouthPrefabs;
	public Transform[] handPrefabs;
	public Transform[] feetPrefabs;
	public Transform[] headGearPrefabs;
	...
	public Transform headGearPoint;
	...
	{						
		...
		//------------------------[ HeadGear ]------------------------------\\		
		// Now let's handle the mouth.
		Transform headGearToSpawn = headGearPrefabs[Random.Range(0, headGearPrefabs.Length)];
		Transform headGear = Instantiate(headGearToSpawn);
		headGear.SetParent(headGearPoint);
		headGear.localPosition = Vector3.zero;
		headGear.localRotation = Quaternion.identity;

		//------------------------[ Hands ]------------------------------\\	
		// We start by picking an hand prefab randomly from our list.
		Transform handToSpawn = handPrefabs[Random.Range(0, handPrefabs.Length)];
		// Now we'll actually instantiate it, ie. make it exist in our scene.
		Transform handLeft = Instantiate(handToSpawn);

		// We set its parent to the reference point we assigned ahead of time,
		// and set both the local position and local rotation (ie. how much our
		// object is offset from our parents) to zero.
		handLeft.SetParent(leftHand);
		handLeft.localPosition = Vector3.zero;
		handLeft.localRotation = Quaternion.identity;

		// Let's do the same for the right hand. We don't need to pick another
		// hand prefab from our list -- we probably want both hands to be the same
		// style!
		Transform handRight = Instantiate(handToSpawn);
		handRight.SetParent(rightHand);
		handRight.localPosition = Vector3.zero;
		handRight.localRotation = Quaternion.identity;

		//------------------------[ Foot ]------------------------------\\	
		// We start by picking an foot prefab randomly from our list.
		Transform footToSpawn = feetPrefabs[Random.Range(0, feetPrefabs.Length)];
		// Now we'll actually instantiate it, ie. make it exist in our scene.
		Transform footLeft = Instantiate(footToSpawn );

		// We set its parent to the reference point we assigned ahead of time,
		// and set both the local position and local rotation (ie. how much our
		// object is offset from our parents) to zero.
		footLeft.SetParent(leftFoot);
		footLeft.localPosition = Vector3.zero;
		footLeft.localRotation = Quaternion.identity;

		// Let's do the same for the right foot. We don't need to pick another
		// foot prefab from our list -- we probably want both feets to be the same
		// style!
		Transform footRight = Instantiate(footToSpawn);
		footRight.SetParent(rightFoot);
		footRight.localPosition = Vector3.zero;
		footRight.localRotation = Quaternion.identity;
		...
		if(Random.value < 0.8f) {
			...
			// 20% of the time, each eyebrow will have a random value instead of sharing the same value.
